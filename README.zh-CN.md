...
> **v0.7.0 — 验证状态(诚实标注)。** 纯分析 + OPC-UA 路径用**真实 in-process asyncua 服务器**测;能源/信创绑定在 2026-06-30 的验证轮里用**真库 + 容器**跑过:**IEC-104**(真 c104 loopback)、**IoTDB** + **TDengine**(真容器写→读 round-trip)**已验证**;HART 命令编解码层对 `hart-protocol` 已验证。**Modbus-RTU(真串口)现已验证**(2026-07-02):用 `socat` PTY 对(软件级 null-modem)+ `pymodbus` RTU 服务器搭真串口链路,连接器的 holding/input/coil/discrete 读操作经真实 RTU 帧 round-trip(`tests/test_modbus_rtu_live.py`);尚未对具体物理 RS-485 设备验证。**BACnet/IP 读路径现已验证**(2026-07-02):在 Linux 容器内以双 IP 同网段搭 **真实 bacpypes3 虚拟 BACnet/IP 设备**,经真实异步 BAC0(2024+)协议栈完成 Who-Is 发现 + present-value 读的真 round-trip(`tests/test_bacnet_live.py`)——异步协程被桥接到独立事件循环,使同步连接器可用于协程 API;真 HVAC 设备上的 BACnet 写/COV/趋势日志仍 `待核实`。**仍 `待核实`(预览、未经硬件验证):** DNP3、IEC-61850(真 IED)、BACnet 写/COV/趋势(真 HVAC)、HART-IP 线传输(真网关)、EtherCAT(无软件仿真器——仅 Linux + root + 真总线)。S7/MC/EtherNet-IP/SECS-GEM 用 mock 客户端;MTConnect 用静态 XML;Sparkplug 用合成 protobuf。详见**安全与治理**。
