...
| | 从哪开始 | 之后 |
|---|---|---|
| **A · 设备直读**（还没有 UNS） | `iaiops scan run` 勘察 → `iaiops onboard draft` 出端点草稿 | 点表 → 声明语义 → 采集 → 分析 |
| **B · 订阅 UNS**（数据已在消息代理上） | 配 mqtt 端点（**必须写 `stale_after_s`**）→ `iaiops mqtt uns-live-audit` 体检命名空间 | 选点（手写 `tags:`）→ 声明语义 → 采集 → 分析 |

两条路在「声明语义」之后合流，后面是同一个脑、同一套治理。**B 路线不扫描** —— 扫描永远认不出 MQTT。

体检结论存在本地，把 UNS 站点分成 **B1（命名空间干净）** 与 **B2（治理本身就是工作）**。
`onboard` 不联网，所以它只认存下来的那份结论；没有就说「未定」，不猜。判 B1 要求那份体检：
按你给的命名标准（`--root` / `--min-segments`）、没被消息上限截断、听满至少一个发布间隔
（`stale_after_s`）、而且就是这个端点**现在**的代理与话题过滤。差一条就不判 B1，并说明差哪条。
只有 `sprawling` 才判 B2；`minor` 是几条启发式发现，列为待复核 —— 一个符合规范的 Sparkplug
命名空间就能触发它。

**为什么 `stale_after_s` 必填**：MQTT 是推送的，采集器读的是最后值缓存。发布者掉线后缓存照样
返回旧值，停了的线会读出 100% 可用率。只有现场知道一个点多久发一次，所以这个值不给默认。
没写就拒绝采集 —— `readiness` 也会把这个端点标成不可采集并写明原因。
...
- **读优先**:**196 个受治理工具 = 183 只读 + 10 个 MOC 设备写 + `historian_push`(也是写,但写的是历史库不是设备,`[WRITE][risk=low]`)+ 2 个已弃用别名**(每版工具只在选中对应 edition 时加载,故裸协议/单 edition 面比该全线总数小);读侧新增两层厂商 REST **只读**面——BAS 控制器层(Metasys/Niagara,building 版)与 Ignition Gateway MES/SCADA 层(factory 版);10 个写/命令工具(`s7_write_db`、`mc_write_words`、`fins_write_words`、`mqtt_publish`、`eip_write_tag`、`ethercat_write_sdo`、`ethercat_set_state`、`profinet_dcp_set`、`bacnet_write_property`、`bas_command`)全部 `[WRITE][risk=HIGH][MOC]`(`bas_command` 默认关闭 + 生命安全对象 denylist)。
