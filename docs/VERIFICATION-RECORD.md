...
| **MQTT / Sparkplug B** | 2a | `test_uns_live_integration.py`, `test_mqtt_retained_undo.py`, `test_uns_tap_live.py` | Real **mosquitto** broker through the full paho loop: UNS audit, Sparkplug schema from NBIRTH, retained-publish BEFORE capture and inverse round-trip. **The collection tap** against a broker on another host, with an edge node publishing to the spec — alias-only NDATA, periodic re-BIRTH, epoch-millisecond metric timestamps: staleness, NDEATH and never-published all become gaps rather than readings, and re-reads of the last-value cache are counted apart from publishes (2026-09-16; six defects found this way, every one invisible to the mocked suite) | Production EoN node / Sparkplug host application — **the edge node is ours, written to the spec, not a commercial one** (待核实); broker auth/TLS; a broker product other than mosquitto |
