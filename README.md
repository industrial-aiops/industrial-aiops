...
- **Validation**: the tap is verified end-to-end against a real **eclipse-mosquitto:2** broker through the full paho loop (`tests/test_uns_tap_live.py`, opt-in) — publish → read → publisher stops → the run records a gap instead of repeating the cache. 待核实: not validated against a production Sparkplug EoN node or a commercial broker (HiveMQ / EMQX).
