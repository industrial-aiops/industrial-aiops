    ...
    def connect(self) -> None:
        ...
        try:
            ...
            if command != FINS_TCP_CMD_SERVER_NODE or len(body) < 8:
                raise FinsFramingError(
                    f"FINS/TCP handshake failed: expected command 1 with 8-byte "
                    f"body, got command {command} with {len(body)} bytes."
                )
            client_node, server_node = struct.unpack(">II", body[:8])
        except Exception:
            # A failed connect/handshake must not leak the socket: the session
            # factory deliberately skips teardown when connect() raises, and in
            # the long-lived MCP server a leaked FD per call is self-reinforcing
            # (0x25 "all node addresses in use" holds server node slots open).
            self.close()  # closes the socket and resets self._sock to None
            raise
