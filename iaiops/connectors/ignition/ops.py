...
the audit log). Because the caller supplies BOTH the destination and the secret
key, the transport runs :mod:`iaiops.core.runtime.url_guard` before any network
I/O: a stored token only rides to an internal host or one the operator
allowlisted via ``IAIOPS_TOKEN_EGRESS_HOSTS`` (stored-token exfiltration
defense).
"""
